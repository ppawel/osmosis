...
public class RelationDao extends EntityDao<Relation> {
	...
	private ReleasableIterator<DbOrderedFeature<RelationMember>> getRelationMembers(String tablePrefix) {
		...
		FileBasedSort<DbOrderedFeature<RelationMember>> sortingStore = new FileBasedSort<DbOrderedFeature<RelationMember>>(
		...
		try {
			...
			// completion.
			resultIterator = new StoreReleasingIterator<DbOrderedFeature<RelationMember>>(sortingStore.iterate(),
					sortingStore);
	...
	protected List<FeaturePopulator<Relation>> getFeaturePopulators(String tablePrefix) {
		...
		// Wrap the way node source into a feature populator that can attach
		// them to their
		// owning ways.
		featurePopulators.add(new FeaturePopulatorImpl<Relation, RelationMember, DbFeature<RelationMember>>(
